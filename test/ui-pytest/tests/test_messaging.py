...
def test_one_to_one_chat(user_data: SystemPath, user_data_two: SystemPath, message):
    ...
    main_window_one.log_in_user(constants.user_account)
    ...
    main_window_two.log_in_user(constants.user_account_two)
    SplashScreen().wait_until_appears().wait_until_hidden(settings.APP_LOAD_TIMEOUT_MSEC)
    main_window_two.minimize()

    aut_one.attach()
    messages_screen = main_window_one.navigator.open_messages_screen()
    chat_view = messages_screen.left_panel.open_create_chat_view().create_chat([constants.user_account_two.name])
    assert driver.waitFor(lambda: chat_view.title == constants.user_account_two.name)
    ...
    driver.waitFor(lambda: constants.user_account.name in messages_screen.left_panel.contacts,
                   settings.APP_LOAD_TIMEOUT_MSEC), f'contact not found in {messages_screen.left_panel.contacts}'
    chat_view = messages_screen.left_panel.open_chat_with(constants.user_account.name)
